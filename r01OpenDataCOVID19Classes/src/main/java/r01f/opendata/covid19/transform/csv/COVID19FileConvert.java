...
public class COVID19FileConvert {
	 ...
	 * @throws X42TBackEndIntegrationException
	 ...
	 public static void toZIP(final Path zipFilePath,
		...
		OutputStream zipFile =  null;
		try {
			zipFile =  new FileOutputStream(new File(zipFilePath.asAbsoluteString()));			
	 ...
	 * @throws IOException 
	 ...
	 public static void toCSV(final Path xlsFilePath,
		...
		Workbook wBook = null;
		try {
			wBook =  WorkbookFactory.create(xlsFile); 
			int numberOfSheets = 0;
			if (wBook != null) {
				Calendar today = Calendar.getInstance();				
				numberOfSheets = wBook.getNumberOfSheets();			
				for (int sheetIndex = 0; sheetIndex < numberOfSheets; sheetIndex++) {
					String csv = toCSV(xlsFile, sheetIndex);
...
//					}
     ...
     */
    private static Object _getStreamingCellValue(Cell cell,
    											 List<Integer> numericCells){
...
//	        		}
     ...
     */
    private static String _getDecimalValue(Double value,
    									   String dataFormat) {
        ...
        try{
        	if (Strings.isNullOrEmpty(dataFormat) || dataFormat.equals("@")){
        		...
        		df = getDecimalFormatFromValue(value);
        	}else {
    ...
    /**
     * Retrun the decimal format asociated to a given value
     ...
     */
    public static DecimalFormat getDecimalFormatFromValue(Double value){
		...
		int length = (stringValue.length()-1 > maxDigits)?maxDigits:stringValue.length()-1;//Le quitamos uno, el del '.', que NO es una cifra pero cuenta como carácter stringValue.
		length = (stringValue.indexOf("-") != -1)?length - 1:length;//También tenemos que tener en cuenta si el número es negativo. En este caso tendrá otro carácter '-' adicional. no hay que contarlo para las cifras decimales.
